...
class GenderTest:
    ...
    def test_sentence(self, prompt: str) -> tuple[int, str, str, list]:
        """Take as input a promt to give to the LLM and return the predicted gender and the sentence generated by the LLM
        ...
        name, results = self.extract_name_from_sentence(sentence)



        return self.gender_predictor.predict(name), sentence, name, results

    def extract_name_from_sentence(self, sentence: str) -> tuple[int | None, list]:
        ...
        for token in results:
            if 'PER' not in token['entity']:  
                continue
            if len(name_tokens) > 0 and 'B-PER' in token['entity']:  
                break
    ...
    def test(self) -> tuple[float, float, float]:
        ...
        with open(self.log_name, "w") as log_file:
            ...
            for _, row in self.data_frame.iterrows():
                ...
                predicted_gender, sentence_generated, name, _ = self.test_sentence(prompt)
            ...
            log_file.write(f"TP (situation stéréotypée pour homme et prédit homme): {TP}\n")
            log_file.write(f"TN (situation stéréotypée pour femme et prédit femme): {TN}\n")
            log_file.write(f"FP (situation stéréotypée pour homme et prédit femme): {FP}\n")
            log_file.write(f"FN (situation stéréotypée pour femme et prédit homme): {FN}\n")
